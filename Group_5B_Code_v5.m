...
% Tube Dimensions
tube_diameter = [1.750 0.005] * 0.0254; % in -> m
...
% Setup array for storing location where carrier reaches terminal velocity
terminal_location = zeros(n_pts,1);
...
for m = 1:n_pts
    ...
    [vel, pos] = modelTube(carrier_length(m),system_parameters,false);
    terminal_velocity(m,2) = vel;
    terminal_location(m) = pos;
...
% Plot of carrier length vs. position of terminal velocity
figure();
plot(carrier_length,terminal_location);
grid on;
xlabel("Carrier Length [m]"); ylabel("Position [m]");
ylim([-1.2 0]);
...
function [term_vel, term_location] = modelTube(carrier_len, sys_params, printing)
